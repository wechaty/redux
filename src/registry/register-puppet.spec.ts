...
test('registerPuppetInRegistry() with store & wechaty', async t => {
  ...
  t.same(spy.args[0]![0], duck.actions.registerPuppet(puppet.id), 'should emit puppet register action after subscribe once')
  t.same(spy.args[1]![0], duck.actions.bindWechatyPuppet({ puppetId: puppet.id, wechatyId: wechaty.id }), 'should emit bind wechaty puppet action after subscribe once')
  ...
  t.same(spy.args[0]![0], duck.actions.unbindWechatyPuppet({ puppetId: puppet.id, wechatyId: wechaty.id }), 'should emit unbind wechaty puppet action after unsubscribed all')
  t.same(spy.args[1]![0], duck.actions.deregisterPuppet(puppet.id), 'should emit deregister puppet action after subscribe all')
})
